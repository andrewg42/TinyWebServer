...
static std::map<std::string_view const, std::string_view const> const url2file =
  {{"/", "../root/register.html"}, {"/0", "../root/register.html"},
   {"/1", "../root/log.html"},     {"/2", "../root/welcome.html"},
   {"/5", "../root/picture.html"}, {"/6", "../root/video.html"}};
...
HTTPConn::HTTPConn(Event_Loop *loop, int fileno)
    : mLoop(loop),
      Socket(fileno),
...
void HTTPConn::read_handler() {
  ...
  while (true) {
    int read_bytes = ::recv(
      get_fd(), (void *)(read_buffer.data() + read_bytes_tot), read_step, 0);
    if (0 ==
        read_bytes) { // socket has been closed, just wait timer to remove it
      ...
      if (read_bytes_tot >= READ_BUFFER_SZ) {
        // any good way?
        LOG_ERROR(" HTTP messsage is too big, {} bytes have been read",
                  read_bytes_tot);
  ...
  http_parser_execute(&parser, &settings, read_buffer.data(),
                      read_buffer.size());
  ...
  if (!rc) { // error on generate response, close socket and wait timer to
             // remove it
...
void Http_Conn::do_request() { // TODO
  ...
  // zero-copy with mmap()
  file_addr = reinterpret_cast<char *>(
...
bool Http_Conn::gen_response() {
  ...
  case HTTP_STATUS_OK: { // 200
    if (0 != file_stat.st_size) {
      if (append_response(_200) && add_headers(file_stat.st_size)) {
        iv[0].iov_base = (void *)write_buffer.data();
        iv[0].iov_len = write_buffer.size();
      ...
      return append_response(_200) && add_headers(_200_form.size()) &&
             append_response(_200_form);
    }
    break;
  }
  case HTTP_STATUS_BAD_REQUEST: { // 400
    return append_response(_400) && add_headers(_400_form.size()) &&
           append_response(_400_form);
  }
  case HTTP_STATUS_NOT_FOUND: { // 404
    return append_response(_404) && add_headers(_404_form.size()) &&
