...
static std::map<std::string_view const, std::string_view const> const url2file = {
    {"/", "../root/register.html"}, {"/0", "../root/register.html"}, {"/1", "../root/log.html"},
    {"/2", "../root/welcome.html"}, {"/5", "../root/picture.html"},  {"/6", "../root/video.html"}};
...
bool Http_Conn::append_response(std::string_view str) { return write_buffer.append(str); }
...
bool Http_Conn::add_content_type() { return append_response("Content-Type:text/html\r\n"); }
...
bool Http_Conn::add_blank_line() { return append_response("\r\n"); }
...
bool Http_Conn::gen_response() {
  ...
  case HTTP_STATUS_OK: { // 200
    if (0 != file_stat.st_size) {
      ...
      return append_response(_200) && add_headers(_200_form.size()) && append_response(_200_form);
    }
    break;
  }
  case HTTP_STATUS_BAD_REQUEST: { // 400
    return append_response(_400) && add_headers(_400_form.size()) && append_response(_400_form);
  }
  case HTTP_STATUS_NOT_FOUND: { // 404
    return append_response(_404) && add_headers(_404_form.size()) && append_response(_404_form);
  }
  default:
    break;
  }
...
void Http_Conn::close_handler() { LOG_DEBUG("Http_Conn::close_handler()"); }

void Http_Conn::error_handler() { LOG_ERROR("http connection error, fd = {}", get_fd()); }
