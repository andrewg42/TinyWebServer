...
int on_method(http_parser *_) {
  if (_->method == HTTP_DELETE)
    _->http_errno = HPE_INVALID_METHOD;
  return 0;
}
...
int on_http_version(http_parser *_, const char *at, size_t length) {
  ...
  if (std::string_view::npos != slash_pos && version_view[slash_pos + 1] == '1' &&
      version_view[slash_pos + 3] == '1') { // only support HTTP/1.1
    _->http_major = 1;
    _->http_minor = 1;
  } else {
    if (_->http_errno == HPE_UNKNOWN)
      _->http_errno = HPE_INVALID_VERSION;
  }
  return 0;
}
...
int on_header_value(http_parser *_, const char *at, size_t length) {
  ...
  if (http_header_field_mp.end() == it)
    return 0;
