...
Event_Loop::~Event_Loop() { ::close(ep_fd); }
...
void Event_Loop::mod_channel(Channel *p_chan) {
  ...
  const Channel_Status cur_status = p_chan->get_status();
...
Timer_Stamp_t Event_Loop::poll() {
  int eve_num =
      ::epoll_wait(ep_fd, events_list.data(), static_cast<int>(events_list.size()), EPOLL_TIMEOUT);
