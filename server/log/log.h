...
#define FOREACH_LOG_LEVEL(f) f(trace) f(debug) f(info) f(critical) f(warn) f(error) f(fatal)
...
#define _FUNCTION(name)                                                                            \
  template <class... Args> void log_##name(std::format_string<Args...> fmt, Args &&... args) {     \
